...
func (s *Sender) Http_PUT_Send(rw http.ResponseWriter, req *http.Request) bool {
	...
	if valid.HasErrors() {
		...
		log.Debug("send wechat receivedID:", receivedID)
		//Sender逻辑开始
		wechatHOST := senderExtend.Info["wechat_host"]
		wcTokenPath := senderExtend.Info["wechat_token_path"]
		wcTemplateSendPath := senderExtend.Info["wechat_template_send_path"]
		appid := senderExtend.Info["appid"]
		secret := senderExtend.Info["secret"]

		var accessToken string

		//请求access_token
		requestURL := wechatHOST + wcTokenPath + "?grant_type=client_credential&appid=" + appid + "&secret=" + secret
		if resp, respErr := gclient.NewClient().Get(requestURL); respErr != nil {
		} else {
		...
		if accessToken != "" {
			...
			if notifyData.Header != "" {
				...
				if headerMap["template_id"] == "" {
					rw.WriteHeader(http.StatusBadRequest)
					log.Error("Unmarshal header template_id is empty")
					return true
				}
			}

			toUserOpenID := notifyData.To
			templateID := headerMap["template_id"]
			jumpURL := headerMap["url"]
			miniprogramData := headerMap["miniprogram"]
			data := notifyData.Plain
			...
			if miniprogramData != "" {
				weData += `"miniprogram":` + miniprogramData + `, `
			}
			...
			log.Debug(weData)
			//发送模版消息
			requestPostURL := wechatHOST + wcTemplateSendPath + "?access_token=" + accessToken
			if res, resErr := gclient.NewClient().Post(requestPostURL, "", strings.NewReader(weData)); resErr != nil {
