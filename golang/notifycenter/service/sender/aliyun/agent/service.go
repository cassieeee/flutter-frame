...
func (s *Sender) Http_PUT_Send(rw http.ResponseWriter, req *http.Request) bool {
	...
	if valid.HasErrors() {
		...
		//获取Info属性
		// infoM := s.getInfo(strconv.Itoa(int(notifyData.Type)), senderObject)
		...
		log.Debug("send aliyun receivedID:", receivedID)
		...
		if notifyData.Header != "" {
			if err := json.Unmarshal([]byte(notifyData.Header), &headerMap); err != nil {
				log.Debug(headerMap)
				rw.WriteHeader(http.StatusBadRequest)
				log.Error("getInfo is empty")
				return true
			}
			if headerMap["sign_name"] == "" {
				log.Error("param sign_name is empty")
				return true
			}
			if headerMap["template_code"] == "" {
				log.Error("param template_code is empty")
				return true
			}
		}

		client, err := sdk.NewClientWithAccessKey(senderExtend.Info["region_id"], senderExtend.Info["access_key_id"], senderExtend.Info["access_secret"])
		log.Debug(err)
		if err != nil {
			log.Error(err)
		}
		request := requests.NewCommonRequest()
		request.Method = senderExtend.Info["method"]
		request.Scheme = senderExtend.Info["scheme"] // https | http
		request.Domain = senderExtend.Info["domain"]
		request.Version = senderExtend.Info["version"]
		request.ApiName = senderExtend.Info["api_name"]
		request.QueryParams["RegionId"] = senderExtend.Info["region_id"]
		request.QueryParams["PhoneNumbers"] = notifyData.To
		request.QueryParams["SignName"] = headerMap["sign_name"]
		request.QueryParams["TemplateCode"] = headerMap["template_code"]
		request.QueryParams["TemplateParam"] = notifyData.Plain

		res, resErr := client.ProcessCommonRequest(request)
		if resErr != nil {
			log.Error("callback res:", res, " error:", resErr)
		} else {
			//Sender逻辑结束
		...
		log.Debug(res.GetHttpContentString())
	}
...
func (s *Sender) getExtend(typ, checkObject string) (extend model.SenderExtendMap) {
	cacheKey := "extend." + checkObject
	if extendCache, found := s.caches.Get(cacheKey); found {
		extend = extendCache.(model.SenderExtendMap)
