...
import (
	...
	"notifycenter/service/sender/aliyun/agent"
	"os"
	"path"
	"runtime"
	"strconv"
...
var (
	confFile = flag.String("f", "./sender-aliyun-agent.conf", "config file")
)
...
func main() {
	...
	if conf.AutoRegister {
		...
		if len(conf.SenderInfo.ExtendInfo) > 0 {
			if gobData, gobErr := ghttp.GobEncode(conf.SenderInfo.ExtendInfo); gobErr != nil {
				log.Error("client:", conf.Client, " GobEncode conf.SenderInfo.ExtendInfo error:", gobErr)
			} else {
				data.Add("info", string(gobData))
			}
		}
