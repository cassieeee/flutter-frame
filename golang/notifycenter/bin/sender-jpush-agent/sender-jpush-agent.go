...
import (
	...
	agent2 "notifycenter/service/sender/jpush/agent"
	"os"
	"path"
	"runtime"
	"strconv"
...
var (
	confFile = flag.String("f", "./sender-jpush-agent.conf", "config file")
)

func main() {
	flag.Parse()
	var conf agent2.Config
	...
	if conf.AutoRegister {
		...
		data.Add("type", strconv.Itoa(int(model.SenderTypeAPP)))
	...
	webrouter.Register("/", agent2.NewSender(
		base,
		caches,
		conf.LogSenderHost,
