...
class BaseResp<T> {
  ...
  BaseResp.listdata(this.code, this.msg, this.ldata);
...
class DioUtil {
  ...
  /// BaseResp [String msg]字段 key, 默认：msg.
  String _msgKey = "msg";
  ...
  Future<BaseResp<T>> request<T>(String method, String path,
    ...
    try {
      ...
      return new BaseResp(500, '网络异常,请检查网络连接~', null);
    }
    ...
    if (response.statusCode == HttpStatus.ok ||
      ...
      try {
        if (response.data is Map) {
          ...
          if (_dataMap[_dataKey] is List)
            _ldata = _dataMap[_dataKey];
          else
            _data = _dataMap[_dataKey];
        }
        // if (_code == 50008) {
        //   Navigator.pushReplacement(AppInstance.initContext,
        //       MaterialPageRoute<void>(builder: (ctx) => LoginPage()));
        // Navigator.pushAndRemoveUntil(
        //   AppInstance.initContext,
        //   MaterialPageRoute<void>(builder: (ctx) => LoginPage()),
        ...
        // }
        if (_data == null)
          return new BaseResp.listdata(_code, _msg, _ldata);
        else
          return new BaseResp(_code, _msg, _data);
      } catch (e) {
        return new BaseResp(response.statusCode, '数据解析异常', null);
    ...
    return new BaseResp(response.statusCode, '服务器异常', null);
    // return new Future.error(new DioError(
    //   response: response,
